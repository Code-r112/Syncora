  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes}>
      ...
      >
        ...
        {/* Track name & artist */}
        <div className="flex-grow min-w-0 ml-3 select-none flex flex-col justify-center">
          <div
            className={cn(
          ...
          >
            {sourceState.source.metadata?.title || extractFileNameFromUrl(sourceState.source.url)}
            {isError && sourceState.error && <span className="text-xs text-red-400 ml-2">({sourceState.error})</span>}
          </div>
          {/* Artist subtext (if available) */}
          {sourceState.source.metadata?.artist && (
            <div className="text-xs text-neutral-500 truncate select-none mt-0.5">
              {sourceState.source.metadata.artist}
            </div>
          )}
        </div>
